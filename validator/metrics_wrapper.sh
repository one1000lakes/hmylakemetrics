...
#Wrapper should be called like this: /data/hmylakemetrics/metrics_wrapper.sh -t MINUTEINTERVAL -p BASEPATH -e HMYPATH -s SHARDTHATNODESIGNS -r REMOTENODEADDRESS -h NODEHASH
#Example: /data/hmylakemetrics/metrics_wrapper.sh -t 1 -p /data/hmylakemetrics -e /home/user/hmy -s 1 -r example.remotenod3.com -h one123456789abcdefghijklmnopqrstuvwxyz
#Example indicates it should run tasks marked to run 1 minute interval and basepath for .sh files and BASEPATH/php directory is /data/hmylakemetrics, location for hmy executable is /home/user/hmy, shard that this node signs is 1, this node's remote node (backup/main) is example.remotenod3.com and validators address is one123456789abcdefghijklmnopqrstuvwxyz
#Wrapper should be called from crontab at 1 and 5 minutes interval
